...
import com.google.ai.edge.gallery.data.PhoneLlamaCatalog
...
/**
 * Models that require HuggingFace account authentication (gated=auto or gated=manual).
...
constructor(
  ...
  // Serializes all model init/cleanup so that rapid A→B→C switches never race against each other.
  // Two concurrent initializeModel coroutines could both try to close the same engine (double-close
  // = native crash). The Mutex ensures only one runs at a time.
  ...
  /** Names of models injected from [PhoneLlamaCatalog] (not from the upstream allowlist). */
  val extendedModelNames: Set<String> = PhoneLlamaCatalog.modelNames
  ...
  fun initializeModel(
    ...
    // Catch OOM and other Throwables that escape the engine init so the process stays alive.
    val oomHandler = CoroutineExceptionHandler { _, throwable ->
      model.initializing = false
      val msg = when (throwable) {
        is OutOfMemoryError -> "Out of memory: '${model.name}' is too large for available RAM. Close other apps and retry, or use a smaller model."
        else -> throwable.message ?: "Unknown error during model initialization"
      }
      Log.e(TAG, "Uncaught error initializing '${model.name}': $msg", throwable)
      System.gc()
      updateModelInitializationStatus(model = model, status = ModelInitializationStatusType.ERROR, error = msg)
    }
    viewModelScope.launch(Dispatchers.Default + oomHandler) {
      // Serialize all init/cleanup so concurrent A→B and B→C switches cannot race each other.
      // Without this mutex, two coroutines could both see the old model loaded and both call
      // engine.close() on it — a double-close that crashes the native runtime.
      initMutex.withLock {
      ...
      // Unload all other initialized models first — only one model should be in memory at a time.
      // cleanUp() is synchronous and runs on Dispatchers.Default (same thread the engine was
      // created on). Do NOT move to Dispatchers.IO — LiteRT engine has thread affinity and
      // crashes if closed from a different thread pool than it was initialized on.
      val otherLoaded = getAllModels().filter { it != model && it.instance != null }
      if (otherLoaded.isNotEmpty()) {
        Log.d(TAG, "Unloading ${otherLoaded.size} other model(s) before initializing '${model.name}'")
        for (other in otherLoaded) {
          try { other.runtimeHelper.stopResponse(other) } catch (_: Throwable) {}
        }
        // Wait for any in-progress inference to actually finish before engine.close().
        // stopResponse() signals cancellation but doesn't block until inference stops.
        // engine.close() called while inference is running will crash the native runtime.
        EdgeServerManager.waitForIdle(10_000L)
        kotlinx.coroutines.delay(200) // small settle after lock released
        for (other in otherLoaded) {
          val otherTask = findTaskForModel(other)
          if (otherTask != null) {
            // cleanUp() is synchronous — this returns only after engine.close() completes
            cleanupModel(context = context, task = otherTask, model = other)
          }
        }
        kotlinx.coroutines.delay(300) // settle after close before loading new model
        Log.d(TAG, "Other models unloaded, proceeding to initialize '${model.name}'")
      }

      // Clean up target model itself (handles partial-init state).
      cleanupModel(context = context, task = task, model = model)

      // Pre-flight: reject models that are empirically too large for this device's GPU allocator.
      // The native LiteRT GPU allocator works at kernel level — if it triggers OOM, Android kills
      // the ENTIRE process before Java can catch it. We cannot recover from that, so we must
      // refuse to attempt loading rather than trying to handle the crash.
      //
      // The limit (default 2900MB) is configurable via POST /admin/config {"max_model_size_mb":N}.
      // Empirical: ≤2.4GB loads fine on Pixel Fold; ≥3.4GB causes a kernel OOM kill every time.
      val limitMb = EdgeServer.globalConfig.maxModelSizeMb
      if (limitMb > 0) {
        val modelSizeBytes: Long = model.sizeInBytes.takeIf { it > 0L }
          ?: try { java.io.File(model.getPath(context)).length() } catch (_: Throwable) { 0L }
        val modelSizeMb = modelSizeBytes / 1024 / 1024
        if (modelSizeBytes > 0L && modelSizeMb > limitMb) {
          val msg = "Model '${model.name}' is ${modelSizeMb}MB — exceeds the ${limitMb}MB safety limit. " +
            "Raise the limit via POST /admin/config {\"max_model_size_mb\":N} if you want to risk loading it."
          Log.e(TAG, msg)
          model.initializing = false
          updateModelInitializationStatus(model = model, status = ModelInitializationStatusType.ERROR, error = msg)
          EdgeServerManager.reportModelLoadError(model.name, msg)
          return@withLock
        }
        if (modelSizeBytes > 0L) {
          Log.d(TAG, "Model size check OK: '${model.name}' is ${modelSizeMb}MB (limit ${limitMb}MB)")
        }
      }
      ...
      val onDoneFn: (error: String) -> Unit = { error ->
        ...
        if (model.instance != null) {
          ...
          // Report failure back to the API server so /health reflects the error
          // and clients don't hang waiting for a model that will never load.
          EdgeServerManager.reportModelLoadError(model.name, error)
        }
      }
  ...
  fun cleanupModel(
    ...
    if (model.instance != null) {
      ...
      // Always call onDone so callers don't hang waiting for a no-op cleanup.
      onDone()
    }
  }
  ...
  fun setDownloadStatus(curModel: Model, status: ModelDownloadStatus) {
    ...
    EdgeServerManager.updateKnownModels(getAllDownloadedModelsForApi())
  }
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          // Skip models that require HuggingFace authentication — they will never download.
          if (allowedModel.modelId != null && GATED_MODEL_IDS.contains(allowedModel.modelId)) {
            Log.d(TAG, "Skipping gated model '${allowedModel.name}' (${allowedModel.modelId})")
            continue
          }
        ...
        // Inject PhoneLlama extended catalog models (not in upstream allowlist).
        val upstreamNames = nameToModel.keys.toSet()
        for (catalogEntry in PhoneLlamaCatalog.EXTENDED_MODELS) {
          if (catalogEntry.name in upstreamNames) continue // already covered by upstream
          val model = catalogEntry.toModel()
          _allowlistModels.add(model)
          nameToModel[model.name] = model
          for (taskType in catalogEntry.taskTypes) {
            curTasks.find { it.id == taskType }?.models?.add(model)
          }
          Log.d(TAG, "PhoneLlama catalog: added extended model '${model.name}'")
        }
        ...
        // Register model switcher so the API can trigger model changes
        EdgeServerManager.setModelSwitcher { requestedName ->
          val downloaded = getAllDownloadedModels()
          val target = downloaded.firstOrNull { model ->
            val id = model.displayName.ifEmpty { model.name }
            id.equals(requestedName, ignoreCase = true) || model.name.equals(requestedName, ignoreCase = true)
          }
          if (target != null) {
            viewModelScope.launch(Dispatchers.Default) {
              // If already initialized just re-bind — don't re-initialize.
              if (target.instance != null) {
                Log.d(TAG, "Model '${target.name}' already initialized — re-binding to API server.")
                EdgeServerManager.bindModel(target, target.runtimeHelper, target.displayName)
              } else {
                // Delegate entirely to initializeModel — it handles stopping inference and
                // unloading all other models via CountDownLatch before loading the target.
                // Do NOT do a separate pre-cleanup here; double-cleanup causes native crashes.
                val task = findTaskForModel(target)
                if (task != null) initializeModel(context = context, task = task, model = target)
              }
            }
            true
          } else false
        ...
        // Register model lister so /v1/models always returns fresh list
        EdgeServerManager.setModelLister {
          getAllDownloadedModelsForApi().map { it.displayName.ifEmpty { it.name } }
        }
  ...
  /** Auto-initializes the first downloaded LLM model at startup for API serving. */
  private fun autoLoadFirstModel() {
    viewModelScope.launch(Dispatchers.Default) {
      // Update API model list with ALL downloaded models (not just LLMs)
      EdgeServerManager.updateKnownModels(getAllDownloadedModelsForApi())
      val downloadedModels = getAllDownloadedModels()
      if (downloadedModels.isEmpty()) {
        Log.d(TAG, "Auto-load: no downloaded models found.")
        return@launch
      }
      val model = downloadedModels.first()
      val task = findTaskForModel(model)
      if (task == null) {
        Log.d(TAG, "Auto-load: task not found for model ${model.name}")
        return@launch
      }
      Log.d(TAG, "Auto-load: initializing model '${model.name}' from task '${task.id}'")
      initializeModel(context = context, task = task, model = model)
    }
  }

  /** On first boot when no models are downloaded, silently starts downloading the first LLM model. */
  fun autoDownloadFirstModelIfNeeded() {
    viewModelScope.launch(Dispatchers.Default) {
      if (getAllDownloadedModels().isNotEmpty()) return@launch
      val taskAndModel =
        uiState.value.tasks
          .filter { it.id == BuiltInTaskId.LLM_CHAT || it.models.any { model -> model.isLlm } }
          .sortedBy { if (it.id == BuiltInTaskId.LLM_CHAT) 0 else 1 }
          .flatMap { task -> task.models.map { model -> task to model } }
          .firstOrNull { (_, model) -> model.isLlm }
      if (taskAndModel == null) {
        Log.d(TAG, "AutoDownload: no LLM model found in allowlist")
        return@launch
      }
      val (task, model) = taskAndModel
      Log.d(TAG, "AutoDownload: starting download of ${model.name}")
      withContext(Dispatchers.Main) { downloadModel(task = task, model = model) }
    }
  }

  private fun findTaskForModel(model: Model): Task? {
    return uiState.value.tasks.firstOrNull { it.id == BuiltInTaskId.LLM_CHAT && it.models.contains(model) }
      ?: uiState.value.tasks.firstOrNull { it.models.contains(model) }
  }
