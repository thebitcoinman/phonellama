...
object LlmChatModelHelper : LlmModelHelper {
  ...
  override fun initialize(
    ...
    try {
      ...
      // OOM during GPU model allocation — common for 3B+ models on constrained devices.
      // Catch here before the JVM propagates it as an uncaught Throwable that kills the process.
      Log.e(TAG, "OOM loading model '${model.name}' — model too large for available memory", e)
      // Free any partially-allocated native resources
      System.gc()
      onDone("Out of memory: model '${model.name}' is too large for available device memory. Try closing other apps and retry, or use a smaller model.")
      return
    } catch (e: Throwable) {
  ...
  override fun resetConversation(
    ...
    try {
      ...
      // skip_close: toggled at runtime via POST /admin/config {"skip_close": true/false}
      // false = correct lifecycle (close old conversation before creating new one)
      // true  = skip nativeDeleteConversation() — leaks one conversation per call but isolates crash
      if (!com.google.ai.edge.gallery.edgeserver.EdgeServer.globalConfig.skipClose) {
        instance.conversation.close()
      } else {
        Log.d(TAG, "resetConversation: skip_close=true, not calling conversation.close()")
      }
  ...
  override fun cleanUp(model: Model, onDone: () -> Unit) {
    ...
    // LiteRT engine has thread affinity — engine.close() must run on the same thread pool
    // (Dispatchers.Default) that created it. Calling from the main thread causes a native crash.
    if (Looper.myLooper() == Looper.getMainLooper()) {
      Log.e(TAG, "cleanUp() called from MAIN THREAD — this will crash! " +
        "All cleanupModel() calls must be dispatched to Dispatchers.Default first.")
    }
  ...
  override fun runInference(
    ...
    // Always update the cleanUpListener for this model — each request has its own doneLatch,
    // so we MUST store the latest listener or stream 2+ doneLatch will never fire → 60s timeout
    // → stopResponse() called on active inference → SIGSEGV.
