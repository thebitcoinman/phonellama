...
fun GlobalModelManager(
  ...
  val edgeGalleryModels = remember { mutableStateListOf<Model>() }
  val extendedModels = remember { mutableStateListOf<Model>() }
  ...
  // Banner permanently dismissed - no Gemma4 marketing
  ...
  LaunchedEffect(uiState.modelImportingUpdateTrigger) {
    ...
    val extendedNames = viewModel.extendedModelNames
    ...
    val nonImported = sortedModels.filter { !it.imported }
    edgeGalleryModels.clear()
    edgeGalleryModels.addAll(nonImported.filter { it.name !in extendedNames })
    extendedModels.clear()
    extendedModels.addAll(nonImported.filter { it.name in extendedNames })
  ...
  Scaffold(
        ...
        title = {
          Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Row(
              ...
              Text(
                text =
                  "${stringResource(R.string.drawer_models_label)} (${edgeGalleryModels.size + extendedModels.size + importedModels.size})",
    ...
    Box() {
      LazyColumn(
        ...
        // ── Edge Gallery Models ──────────────────────────────────────────
        item(key = "edge_gallery_models_label") {
          Text(
            stringResource(R.string.model_list_edge_gallery_models_title),
            color = MaterialTheme.colorScheme.primary,
            style = MaterialTheme.typography.labelLarge,
            modifier = Modifier.padding(horizontal = 16.dp).padding(top = 8.dp, bottom = 8.dp),
          )
        }
        items(edgeGalleryModels) { model ->
        ...
        // ── PhoneLlama Extended ──────────────────────────────────────────
        item(key = "extended_models_label") {
          androidx.compose.foundation.layout.Column(
            modifier = Modifier.padding(horizontal = 16.dp).padding(top = 32.dp, bottom = 4.dp),
          ) {
            Text(
              stringResource(R.string.model_list_extended_models_title),
              color = MaterialTheme.colorScheme.primary,
              style = MaterialTheme.typography.labelLarge,
            )
            Text(
              stringResource(R.string.model_list_extended_models_subtitle),
